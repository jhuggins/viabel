...
from viabel.patterns import SimplexArrayPattern, NumericArrayPattern, PatternDict, PSDSymmetricMatrixPattern
...
def get_test_pattern():
    ...
    pattern = PatternDict()
    pattern['array'] = NumericArrayPattern(
        (2, 3, 4), lb=-1, ub=20, default_validate=False)
    pattern['mat'] = PSDSymmetricMatrixPattern(
        3, default_validate=False)
    pattern['simplex'] = SimplexArrayPattern(
        2, (3, ), default_validate=False)
    subdict = PatternDict()
    subdict['array2'] = NumericArrayPattern(
...
class TestFlatteningAndFolding(unittest.TestCase):
    def _test_transform_input(
        ...
        fun_trans = function_patterns.TransformFunctionInput(
    ...
    def _test_transform_output(
        ...
        fun_trans = function_patterns.TransformFunctionOutput(
    ...
    def test_transform_input(self):
        ...
        for free, origflat in itertools.product(ft, ft):
            ...
            if origflat:
                fold_tf1 = function_patterns.FoldFunctionInput(
                    tf1, pattern, free, 0)
                assert_array_almost_equal(
                    fold_tf1(param_val), tf1(param_flat))
            else:
                flat_tf1 = function_patterns.FlattenFunctionInput(
                    tf1, pattern, free, 0)
                jnp.allclose(
            ...
            with self.assertRaises(ValueError):
                fun_flat = function_patterns.TransformFunctionInput(
    ...
    def test_transform_output(self):
        ...
        for free, origflat in itertools.product(ft, ft):
            ...
            if origflat:
                fold_tf1 = function_patterns.FoldFunctionOutput(
                    tf1, pattern, free, 0)
                jnp.allclose(
                    pattern.flatten(fold_tf1(), free=free), tf1())
            else:
                flat_tf1 = function_patterns.FlattenFunctionOutput(
                    tf1, pattern, free, 0)
                jnp.allclose(
            ...
            # Test bad inits
            with self.assertRaises(ValueError):
                fun_flat = function_patterns.TransformFunctionOutput(
                    tf1, [[ pattern ]], free, origflat, 0)
    ...
    def test_flatten_and_fold(self):
        ...
        folded_fun = function_patterns.FoldFunctionInputAndOutput(
        ...
        flat_fun = function_patterns.FlattenFunctionInputAndOutput(
