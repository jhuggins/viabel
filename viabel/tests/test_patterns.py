...
from viabel.patterns import Pattern, _unconstrain_simplex_matrix, _unconstrain_simplex_jacobian, _constrain_simplex_jacobian, _constrain_simplex_matrix,\
    SimplexArrayPattern, NumericArrayPattern, NumericScalarPattern,NumericVectorPattern,PatternDict, PatternArray,save_folded, \
    load_folded,register_pattern_json, get_pattern_from_json, PSDSymmetricMatrixPattern, _unvectorize_ld_matrix, _vectorize_ld_matrix

from jax.test_util import check_grads

# A pattern that matches no actual types for causing errors to test.
class BadTestPattern(Pattern):
    def __init__(self):
        pass
...
class TestBasicPatterns(unittest.TestCase):
    def test_simplex_jacobian(self):
        ...
        jac_ad = jax.jacrev(_unconstrain_simplex_matrix, allow_int = True)(simplex)
        jac = _unconstrain_simplex_jacobian(simplex)
        assert_array_almost_equal(jac_ad, jac, decimal=5)

        simplex_free = _unconstrain_simplex_matrix(simplex)
        jac_ad = jax.jacrev(_constrain_simplex_matrix, allow_int = True)(simplex_free)
        jac = _constrain_simplex_jacobian(simplex)
    ...
    def test_simplex_array_patterns(self):
        def test_shape_and_size(simplex_size, array_shape):
            ...
            pattern = SimplexArrayPattern(simplex_size, array_shape)
        ...
        self.assertTrue(
            SimplexArrayPattern(3, (2, 3)) != SimplexArrayPattern(3, (2, 4)))

        self.assertTrue(
            SimplexArrayPattern(4, (2, 3)) !=
            SimplexArrayPattern(3, (2, 3)))

        pattern = SimplexArrayPattern(5, (2, 3))
    ...
    def test_numeric_array_patterns(self):
        for test_shape in [(1, ), (2, ), (2, 3), (2, 3, 4)]:
            ...
            pattern = NumericArrayPattern(test_shape)
            _test_pattern(self, pattern, valid_value)

            pattern = NumericArrayPattern(test_shape, lb=-1)
            _test_pattern(self, pattern, valid_value)

            pattern = NumericArrayPattern(test_shape, ub=2)
            _test_pattern(self, pattern, valid_value)

            pattern = NumericArrayPattern(test_shape, lb=-1, ub=2)
            _test_pattern(self, pattern, valid_value)

        # Test scalar subclass.
        pattern = NumericScalarPattern()
        _test_pattern(self, pattern, 2.)

        pattern = NumericScalarPattern(lb=-1)
        _test_pattern(self, pattern, 2.)

        pattern = NumericScalarPattern(ub=3)
        _test_pattern(self, pattern, 2.)

        pattern = NumericScalarPattern(lb=-1, ub=3)
        _test_pattern(self, pattern, 2.)

        # Test vector subclass.
        valid_vec = np.random.random(3)
        pattern = NumericVectorPattern(length=3)
        _test_pattern(self, pattern, valid_vec)

        pattern = NumericVectorPattern(length=3, lb=-1)
        _test_pattern(self, pattern, valid_vec)

        pattern = NumericVectorPattern(length=3, ub=3)
        _test_pattern(self, pattern, valid_vec)

        pattern = NumericVectorPattern(length=3, lb=-1, ub=3)
        _test_pattern(self, pattern, valid_vec)

        # Test equality comparisons.
        self.assertTrue(
            NumericArrayPattern((1, 2)) !=NumericArrayPattern((1, )))

        self.assertTrue(
            NumericArrayPattern((1, 2)) != NumericArrayPattern((1, 3)))

        self.assertTrue(
            NumericArrayPattern((1, 2), lb=2) != NumericArrayPattern((1, 2)))

        self.assertTrue(
            NumericArrayPattern((1, 2), lb=2, ub=4) != NumericArrayPattern((1, 2), lb=2))

        # Check that singletons work.
        pattern = NumericArrayPattern(shape=(1, ))
        _test_pattern(self, pattern, 1.0)

        # Test invalid values.
        with self.assertRaisesRegex(
            ValueError, 'ub must strictly exceed lower bound lb'):
            pattern = NumericArrayPattern((1, ), lb=1, ub=-1)
        ...
        # Test flat indices.
        pattern = NumericArrayPattern((2, 3, 4), lb=-1, ub=1)
        _test_array_flat_indices(self, pattern)

    def test_psdsymmetric_matrix_patterns(self):
        dim = 3
        valid_value = np.eye(dim) * 3 + np.full((dim, dim), 0.1)
        pattern = PSDSymmetricMatrixPattern(dim)
        _test_pattern(self, pattern, valid_value)

        pattern = PSDSymmetricMatrixPattern(dim, diag_lb=0.5)
        _test_pattern(self, pattern, valid_value)

        self.assertTrue(
            PSDSymmetricMatrixPattern(3) != PSDSymmetricMatrixPattern(4))

        self.assertTrue(PSDSymmetricMatrixPattern(3))
        ...
        with self.assertRaisesRegex(ValueError, 'diagonal lower bound'):
            PSDSymmetricMatrixPattern(3, diag_lb=-1)

        pattern = PSDSymmetricMatrixPattern(3, diag_lb=0.5)
...
class TestContainerPatterns(unittest.TestCase):
    def test_dictionary_patterns(self):
        ...
        dict_pattern = PatternDict()
        dict_pattern['a'] = NumericArrayPattern((2, 3, 4), lb=-1, ub=2)
        test_pattern(dict_pattern, dict_pattern.random())

        print('dictionary pattern test: two elements')
        dict_pattern['b'] = NumericArrayPattern((5, ), lb=-1, ub=10)
        test_pattern(dict_pattern, dict_pattern.random())

        print('dictionary pattern test: third matrix element')
        dict_pattern['c'] = PSDSymmetricMatrixPattern(size=3)
        test_pattern(dict_pattern, dict_pattern.random())

        print('dictionary pattern test: sub-dictionary')
        subdict = PatternDict()
        subdict['suba'] = NumericArrayPattern((2, ))
        ...
        with self.assertRaises(ValueError):
            dict_pattern['new'] = NumericArrayPattern((4, ))

        with self.assertRaises(ValueError):
            dict_pattern['a'] = NumericArrayPattern((4, ))
    ...
    def test_pattern_array(self):
        array_pattern = NumericArrayPattern(
            shape=(4, ), lb=-1, ub=10.0)
        pattern_array = PatternArray((2, 3), array_pattern)
        valid_value = pattern_array.random()
        _test_pattern(self, pattern_array, valid_value)

        matrix_pattern = PSDSymmetricMatrixPattern(size=2)
        pattern_array = PatternArray((2, 3), matrix_pattern)
        valid_value = pattern_array.random()
        _test_pattern(self, pattern_array, valid_value)

        base_pattern_array = PatternArray((2, 1), matrix_pattern)
        pattern_array_array = PatternArray((1, 3), base_pattern_array)
        ...
        self.assertTrue(
            PatternArray((3, 3), matrix_pattern) !=
            PatternArray((2, 3), matrix_pattern))

        self.assertTrue(
            PatternArray((2, 3), array_pattern) !=
            PatternArray((2, 3), matrix_pattern))
...
class TestJSONFiles(unittest.TestCase):
    def test_json_files(self):
        pattern = PatternDict()
        pattern['num'] = NumericArrayPattern((1, 2))
        pattern['mat'] = PSDSymmetricMatrixPattern(5)
        ...
        save_folded(outfile_name, val_folded, pattern, extra=extra)

        val_folded_loaded, pattern_loaded, data = load_folded(outfile_name + '.npz')
    ...
    def test_register_json_pattern(self):
        ...
        with self.assertRaisesRegex(
            ...
            get_pattern_from_json(bad_pattern_json)
        with self.assertRaisesRegex(
...
class TestHelperFunctions(unittest.TestCase):
    ...
    def test_pdmatrix_custom_autodiff(self):
        ...
        x_mat = _unvectorize_ld_matrix(x_vec)

        check_grads(_vectorize_ld_matrix,(x_mat,),
            modes=['rev'], order=3)
        check_grads(_unvectorize_ld_matrix,(x_vec,),
            modes=['rev'], order=3)
